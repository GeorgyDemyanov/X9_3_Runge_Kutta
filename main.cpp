...
std::vector<double> f(double time, std::vector<double> sol) {
    int mu = 1000;
    return {sol[1], mu * (1 - sol[1] * sol[1]) * sol[1] - sol[0]};
}

int main() {
    std::vector<std::pair<double, double>> sol = solution(0, 1000, 0.01, {0, 0.001}, f);
    std::ofstream fout(
            "C:\\Users\\Demia\\CLionProjects\\X9.3 Runge_Kutta\\data.txt");
